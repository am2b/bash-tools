...
#=pack
#@分别打包给定目录下的每个视频文件
#@如果有字幕文件的话,会把字幕和视频打包到一个包里面,所以要求视频文件和字幕文件的名称中不要包含无谓的"."字符,比如不要这样:tit.le.mp4或者subt.itle.en.srt等,但是这样的名称是OK的:title.mp4或者subtitle.en.srt等
#@打包结束后,打包后的文件夹会和原始的文件夹处于并列的位置,并且包含一个打包明细的txt文件
#@usage:
#@script.sh dir
...
generate_random_name() {
    ...
    if [[ ! -d "${pack_videos_data_dir}" ]]; then
        mkdir -p "${pack_videos_data_dir}"
    fi

    local unique_filenames_file
    unique_filenames_file="${pack_videos_data_dir}/unique_filenames"
...
main() {
    ...
    #进入目标目录
    cd "${videos_dir}" || { echo "错误:无法进入指定目录:${videos_dir}"; exit 1; }
    ...
    while IFS= read -r -d '' file; do
        ...
        #添加文件到分组
        if [[ -n "${videos_groups[$key]}" ]]; then
            videos_groups["$key"]+=$'\n'"$file"
        else
            videos_groups["$key"]="$file"
        fi
    ...
    local record_file
    record_file=$(basename "${videos_dir}").txt

    for key in "${!videos_groups[@]}"; do
        ...
        IFS=$'\n' read -d '' -r -a files <<<"${videos_groups[$key]}"
    ...
    storage_dir="${videos_dir}-${random_name}"
    mkdir -p "${storage_dir}"
    mv ./*.7z "${storage_dir}"
    ...
    local pack_videos_data_dir="${XDG_DATA_HOME:-$HOME/.local/share}/pack_videos"
    echo "${storage_dir}" > "${pack_videos_data_dir}/storage"
}
